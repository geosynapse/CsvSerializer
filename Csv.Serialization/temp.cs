...
namespace Enstoa.Winthrop.X12.EDIInterface.Utils
{
	public class CsvSerializer<T> where T : class, new()
	{
		...
		private string _newlineReplacement = ((char)254).ToString();

		private List<PropertyInfo> _properties;

		private string _replacement = ((char)255).ToString();
		...
		public CsvSerializer()
		{
			...
			var r = from a in q
					where (a.GetCustomAttributes(typeof(CsvIgnoreAttribute), true).Count() == 0)
					select a;
